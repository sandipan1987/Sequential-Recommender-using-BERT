...
class Dataset(torch.utils.data.Dataset):
    def __init__(self, groups, grp_by, split, history_size=get_params("history_size")):
        self.groups = groups
        self.grp_by = grp_by
        self.split = split
...
def train(
    ...
    history_size: int = get_params("history_size"),
):
    ...
    train_loader = DataLoader(
        ...
        num_workers=get_params("train_num_workers"),
        shuffle=True,
    )
    val_loader = DataLoader(
        val_data,
        batch_size=batch_size,
        num_workers=get_params("val_num_workers"),
        shuffle=False,
    )

    model = Recommender(
        vocab_size=len(mapping) + 2,
        lr=get_params("learning_rate"),
        dropout=get_params("dropout")
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data_csv_path" ,default=get_params("ratings_file_path"))
    parser.add_argument("--epochs", type=int, default=get_params("batch_size"))
